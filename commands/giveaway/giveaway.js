  ...
  run: async (client, message, args) => {
    ...
    async function giveaway() {
      ...
      if (!message.content.split(" ")[1])
        return message.channel.send(
          "Please follow the format. example : ``g!giveaway 1h 773665434612138024 1 Month Discord Nitro``."
        );
      ...
      if (started_time_duration < 1)
        return message.channel.send(
          "The duration time has to be either a minutes or hours **(m or h)**."
        );
      if (
        ...
        return message.channel.send(
          "Please enter a valid id of the channel you want the giveaway to be sent."
        );
      if (prize === "")
