...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let myrank =
      data.map(m => m.ID).indexOf(`xp_${message.author.id}_${message.guild.id}`) + 1 || "N/A";
    data.length = 20;
    let lb = [];
    for (let i in data) {
      let id = data[i].ID.split(`_${message.guild.id}`)[1];
      let user = await client.users.fetch(id);
      user = user ? user.tag : "Unknown User#0000";
      let rank = data.indexOf(data[i]) + 1;
      let level = 3//db.get(`xp_${id}`);
      let xp = data[i].data;
    ...
    lb.forEach(d => {
      embed.addField(`${d.rank}. ${d.user.tag}\n ${d.xp} exp\n level ${d.level}`);
    });
