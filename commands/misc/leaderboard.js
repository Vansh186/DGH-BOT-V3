module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    data = data
      .filter(i => i.ID.startsWith(`xp_${message.guild.id}`))
      .sort((a, b) => b.data - a.data);
    if (data.length < 1)
      return message.channel.send("No members to be on the leaderboard yet!");
    data.length = 20;
    let lb = [];
    for (let i in data) {
    ...
    const embed = new Discord.MessageEmbed()
      .setAuthor(
        `${message.guild.name} - Top 20 Leaderboard`,
        message.guild.iconURL()
      )
      .setColor("RANDOM");
    lb.forEach(member => {
      embed.addField(
        `${member.rank}. ${member.user.tag}`,
        `**Level** - ${member.level}\n**XP** - ${member.xp.replace(
          /['"]+/g,
          ""
        )} / ${member.xpreq}`,
        true
